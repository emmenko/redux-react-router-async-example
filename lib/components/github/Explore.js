...
export default class Explore extends React.Component {
  ...
  render () {
    const err = this.props.github.error
    const errId = Object.keys(err).length ?
      err.repo ? `${err.username}/${err.repo}` : `${err.username}` :
      ''
        ...
        <form className="explore pure-form" onSubmit={e => e.preventDefault()}>
          <fieldset>
            ...
            </button>
            {
              errId ?
                <p className="error">
                  {`Oops, can't find ${errId}: ${err.status} ${err.statusText}`}
                </p> : ''
            }
          </fieldset>
        </form>
